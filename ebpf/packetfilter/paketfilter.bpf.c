...
int xdp_prog_func(struct xdp_md *ctx) {
    // check whether the received packet is an IP packet
    ...
    if (action == XDP_PASS) {
        ipbuf = bpf_ringbuf_reserve(&passed_packets, sizeof(struct src_ip_type), 0);
    } else {
        ipbuf = bpf_ringbuf_reserve(&dropped_packets, sizeof(struct src_ip_type), 0);
    }
