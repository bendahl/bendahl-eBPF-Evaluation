...
struct src_ip_type {
    bool passed;
    // this field has to be called something other than "type", since "type" is a reserved word in Rust
    enum ip_type iptype;
    union {
...
static __always_inline bool ip_is_allowed(struct src_ip_type *ip_src_addr) {
    if(ip_src_addr->iptype == IPV4) {
        bool *ipv4default = (bool *)bpf_map_lookup_elem(&default_config, &ip_src_addr->iptype);
        ...
        bool *ipv6default = (bool *)bpf_map_lookup_elem(&default_config, &ip_src_addr->iptype);
